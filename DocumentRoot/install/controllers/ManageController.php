            ...
            foreach ($packages as $package) {
                ...
                if ($sku) {
                    try {
                        ...
                        $warehouse = new Engine_Warehouse('/purchase/validate');
                        $response = $warehouse->get(array(
                            'sku' => $sku,
                            'key' => $row['value']
                        ));

                        if (isset($response['isValidPurchase']) && $response['isValidPurchase'] === false) {
                            throw new Exception('No purchase found for license key "' . $row['value'] . '".');
                        }
                    } catch (Exception $e) {
                        $this->view->warehouseErrors[$package->getName()] = $e->getMessage();
                    }
                }
            }
        }
