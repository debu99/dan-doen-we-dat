 ...
 * @version    $Id: preview.tpl 2020-01-17 00:00:00 socialnetworking.solutions $
...
<div class="ememsub_pricing_table_preview">
<div class="ememsub_pricing_table_main _bxs ememsub_clearfix <?php echo $this->template->overlap ? 'ememsub_pricing_table_overlap' : ''; ?>" style="background-color:#<?php echo $this->template->body_container_clr; ?>;">
  <div class="ememsub_pricing_table_header" style="background-color:#<?php echo $this->template->header_bgclr; ?>;">
    <p class="ememsub_pricing_table_heading" style="color:#<?php echo $this->template->header_txtclr; ?>;"><?php echo $settings->getSetting('ememsub.table.title',''); ?></p>
    <p class="ememsub_pricing_table_des" style="color:#<?php echo $this->template->header_txtclr; ?>;"><?php echo $settings->getSetting('ememsub.table.description',''); ?></p>
  </div>
  <div class="ememsub_pricing_table">
    <?php foreach($packages as $package): ?>
      ...
      <?php $styleTable = Engine_Api::_()->getDbtable('styles', 'ememsub');
          $column = $styleTable->getStyleId($package->package_id,$this->template->template_id); 
      ?>
      <div class="ememsub_pricing_table_item<?php if(!empty($column->show_highlight)):?> heighlighted<?php endif;?>" style="width:<?php echo is_numeric($column->column_width) ? $column->column_width.'px' : $this->width ?>; <?php if($column->column_margin):?>margin-left:<?php echo $column->column_margin - 4;?>px;margin-right:<?php echo $column->column_margin;?>px;<?php endif;?>">
        <article style="background-color:#<?php echo $column->column_row_color;?>;w">
          ...
          <div class="ememsub_pricing_table_footer">
            ...
            <a href="javascript:;" class="ememsub_animation" onclick="onFormSubmit(<?php echo $package->package_id ?>)" style="background-color:#<?php echo $column->footer_bg_color?>;color:#<?php echo $column->footer_text_color;?>"><?php echo $column->footer_text;?></a>
          </div>
          <!-- PT-FOOTER END --> 
