...
class Sesevent_OrderController extends Core_Controller_Action_Standard {
	public function init() {
		...
		if ($order) Engine_Api::_()->core()->setSubject($order);
		else return $this->_forward('requireauth', 'error', 'core');
	}
	public function indexAction(){
		$order_id = $this->_getParam('order_id', null);
		if(!$order_id) return $this->_forward('requireauth', 'error', 'core');
		$id = $this->_getParam('event_id', null);
		
		$event_id = Engine_Api::_()->getDbtable('events', 'sesevent')->getEventId($id);
		$event = null;

    	if ($event_id) $event = Engine_Api::_()->getItem('sesevent_event', $event_id);
      	if ($event)  $this->view->event = $event ;
      	else return $this->_forward('requireauth', 'error', 'core');

		$this->view->order = $order = Engine_Api::_()->core()->getSubject();
		if($order->state == 'complete')return $this->_forward('notfound', 'error', 'core');
		...
		if(!$ticketDetails)return $this->_forward('requireauth', 'error', 'core');
	}
	public function checkoutAction(){
		...
		if(!$event) return $this->_forward('requireauth', 'error', 'core');
  ...
  public function finishAction() {
	...
	public function successAction(){
		...
		if (!$order || $order->owner_id != $viewer->getIdentity()) return $this->_forward('notfound', 'error', 'core');
		if(!$order_id) return $this->_forward('notfound', 'error', 'core');
    	...
    	if ($event_id) $event = Engine_Api::_()->getItem('sesevent_event', $event_id);
      	if ($event) $this->view->event = $event ;
    	else return $this->_forward('notfound', 'error', 'core');

		$state = $this->_getParam('state');
	  	if(!$state) return $this->_forward('notfound', 'error', 'core');
		...
		if(!$this->view->getTicketCount) return $this->_forward('notfound', 'error', 'core');
	}
	public function viewAction(){
		...
		if(!$event) return $this->_forward('notfound', 'error', 'core');
		$this->view->order = $order = Engine_Api::_()->core()->getSubject();
		$this->view->viewer = $viewer = Engine_Api::_()->user()->getViewer();
