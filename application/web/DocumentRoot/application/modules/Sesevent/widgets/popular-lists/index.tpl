...
<?php if(count($this->results) > 0) :?>
  <ul class=" sesevent_clist_listing sesbasic_bxs sesbasic_clearfix">
    <?php foreach( $this->results as $item ):  ?>
      ...
      <div class="sesevent_clist_item_thumb floatL" style="height:200px;">
...
<?php elseif($this->showType == 'carouselview'):  ?>
 ...
 ?>
  ...
  <style>
    #listslide_<?php echo $randonNumber; ?> {
      position: relative;
      overflow: hidden;
    }
  </style>
  ...
  <div class="slide sesbasic_carousel_wrapper sesbm clearfix sesbasic_bxs <?php if($this->viewType == 'horizontal'): ?> sesbasic_carousel_h_wrapper <?php else: ?> sesbasic_carousel_v_wrapper <?php endif; ?>">
    <div id="listslide_<?php echo $randonNumber; ?>">
    <?php foreach( $this->results as $item ):  ?>
      <div class="sesevent_clist_item sesbasic_clearfix sesbm sesevent_grid_btns_wrap"style="width:<?php echo $this->width ?>px;">
        <div class="sesevent_clist_item_header sesbasic_clearfix">
          <?php if(!empty($this->information) && in_array('postedby', $this->information)): ?>
            <div class="sesevent_clist_item_owner floatL">
              <?php echo $this->htmlLink($item->getOwner()->getHref(), $this->itemPhoto($item->getOwner(), 'thumb.icon', $item->getOwner()->getTitle()), array('title'=>$item->getOwner()->getTitle())) ?>
  ...
  var height_<?php echo $randonNumber; ?> = jqueryObjectOfSes('#listslide_<?php echo $randonNumber; ?>').find('div').first().height();
  jqueryObjectOfSes('#listslide_<?php echo $randonNumber; ?>').css('height', (height_<?php echo $randonNumber; ?> + 14) +'px' );
    ...
    carousel = new Carousel.Extra({
      ...
      container: 'listslide_<?php echo $randonNumber; ?>',
      circular: true,
      current: 1,
